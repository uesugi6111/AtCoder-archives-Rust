    ...
    for i in 0..t {
        ...
        let use_index = {
            ...
            if reset.is_some() {
                ...
                let a = tw.iter().find_position(|x| x.0 == 4).map(|x| x.0);
        ...
        let target = if tw[use_index].0 == 0 {
            ...
            yy % m}else if hv.iter().find(|x|x.0<((t - 1) - i - 1) as i64 * 2i64.pow(capital_increase_count) + tw[use_index].1 ).is_none()
             {
        ...
        let get_index = {
            ...
            if reset.is_some()
                ...
                && i - last_reset > *RESET_INTERVAL
                && (i * 100 < t * *FINAL_PHASE_FOR_RESET)
            {
                reset.unwrap().0
            } else {
