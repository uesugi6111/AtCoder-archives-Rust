...
#[proconio::fastout]
fn main() {
    input!(n: usize, m: usize, s: Chars, c: [usize; n]);

    let mut mm = vec![std::collections::VecDeque::new(); m];

    for i in 0..n {
        mm[c[i] - 1].push_back(s[i]);
    }

    for i in 0..m {
        let buff = mm[i].pop_back();
        if let Some(x) = buff {
            mm[i].push_front(x);
        }
    }
    ...
    for i in 0..n {
        print!("{}", mm[c[i] - 1].pop_front().unwrap());
    }
