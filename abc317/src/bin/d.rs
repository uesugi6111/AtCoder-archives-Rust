...
#[proconio::fastout]
fn main() {
    input!(n: usize, xyz: [(i64, i64, i64); n]);

    let a = xyz.iter().filter(|x| x.0 < x.1).map(|x| x.2).sum::<i64>();
    let b = xyz.iter().filter(|x| x.0 > x.1).map(|x| x.2).sum::<i64>();
    ...
    let min = map.range(c..).map(|x| *x.1).min().unwrap_or(0);
