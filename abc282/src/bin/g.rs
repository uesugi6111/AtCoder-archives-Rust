...
use std::io::Write;
fn main() {
    input!(_n: usize, q: usize, t: Chars, ck: [(u32, u32); q]);
    ...
    for (c, k) in ck {
        match c {
            ...
            2 => {
                writeln!(&mut out, "{}", if bt.count(k) != 0 { 1 } else { 0 }).ok();
            }
            3 => {
                let x = bt.count_less(k);

                if let Some(x) = bt.xth_element(x + 1) {
                    writeln!(&mut out, "{}", x).ok();
                } else {
                    writeln!(&mut out, "-1",).ok();
                }
            }
            _ => {
                let x = bt.count_less(k + 1);
                if let Some(x) = bt.xth_element(x) {
                    writeln!(&mut out, "{}", x).ok();
                } else {
                    writeln!(&mut out, "-1",).ok();
                }
