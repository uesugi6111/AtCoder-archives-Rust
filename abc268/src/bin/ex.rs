...
#[proconio::fastout]
fn main() {
    input!(s: Chars, n: usize, t: [Chars; n]);

    let mut aho = aho::AhoCorasick::new();
...
mod aho {
        ...
        T: Eq + Hash + Copy,
    {
        ...
        pub(crate) fn inner_next(&self, aho_index: usize, value: &T) -> usize {
            return if let Some(&next_index) = self.get_to(aho_index).get(value) {
                next_index
            } else {
                0
            };
