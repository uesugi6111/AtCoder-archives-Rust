...
fn main() {
    ...
    let mut uf = dsu::DisjointSetUnion::new(n);

    for &(a, b) in ab.iter() {
        if uf.is_same(a as usize, b as usize) {
            println!("-1");
            return;
        }
        uf.unite(a as usize, b as usize);
    }

    let mut m_set = std::collections::BTreeMap::new();

    for &(a, b) in ab.iter() {
        m_set.entry(b).or_insert_with(|| vec![]).push(a);
    }
