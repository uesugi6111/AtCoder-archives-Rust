...
fn main() {
    input!(n: usize);
    let mut one_set = std::collections::BTreeSet::new();
    let mut two_set = std::collections::BTreeSet::new();
    let mut three_set = std::collections::BTreeSet::new();

    for i in 1..18 {
        let one = "1".repeat(i).parse::<i64>().unwrap();
        one_set.insert(one);
        let mut v = vec![];
        for k in &one_set {
            v.push(k + one);
    ...
    println!("{}", three_set.iter().skip(n - 1).next().unwrap());
}
