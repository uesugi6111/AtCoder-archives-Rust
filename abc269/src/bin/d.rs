...
fn main() {
    ...
    xyi.sort_by(|a, b| match a.0.cmp(&b.0) {
    ...
    for i in 0..n {
        for j in i + 1..n {
            if xyi[i].0 + 1 < xyi[j].0 {
                break;
            }

            if xyi[i].0 == xyi[j].0 {
                if (xyi[i].1 - xyi[j].1).abs() <= 1 {
                    dsu.unite(xyi[i].2, xyi[j].2);
                }
            }

            if xyi[i].0 + 1 == xyi[j].0 {
                if xyi[i].1 == xyi[j].1 || xyi[i].1 + 1 == xyi[j].1 {
                    dsu.unite(xyi[i].2, xyi[j].2);
                }
            }
        }
    ...
    xyi.sort_by(|a, b| match a.1.cmp(&b.1) {
    ...
    for i in 0..n {
        for j in i + 1..n {
            if xyi[i].1 + 1 < xyi[j].1 {
                break;
            }

            if xyi[i].1 == xyi[j].1 {
                if (xyi[i].0 - xyi[j].0).abs() <= 1 {
                    dsu.unite(xyi[i].2, xyi[j].2);
                }
            }

            if xyi[i].1 + 1 == xyi[j].1 {
                if xyi[i].0 == xyi[j].0 || xyi[i].0 + 1 == xyi[j].0 {
                    dsu.unite(xyi[i].2, xyi[j].2);
                }
            }
        }
    }
    println!("{}", dsu.get_all_groups().len());
}
