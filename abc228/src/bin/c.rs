...
#[proconio::fastout]
fn main() {
    input!(n: usize, k: i64, p: [(i64, i64, i64); n]);

    let pv = p.iter().map(|&x| x.0 + x.1 + x.2).collect::<Vec<_>>();
    let mut pvv = pv.clone();
    pvv.sort_unstable_by(|a, b| b.cmp(a));
    let bs = BinarySearch::new(&pvv, -1, pvv.len() as i64);

    for &v in pv.iter() {
        let ans = bs.search(|x, j| x[j as usize] > v + 300);
