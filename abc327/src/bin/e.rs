...
#[proconio::fastout]
fn main() {
    input!(n: usize, n: usize, k: i64, uvw: [(usize, usize, i64); n]);

    let v = uvw
        .iter()
        .map(|x| mst::Edge(x.0 as i64, x.1 as i64, x.2))
        .collect::<Vec<_>>();

    let ans = mst::kruskal(n, &v, k);
    println!("{}", ans);
}

pub mod mst {
    use super::dsu::DisjointSetUnion;
    pub struct Edge(pub i64, pub i64, pub i64);

    pub fn kruskal(n: usize, edges: &[Edge], k: i64) -> i64 {
