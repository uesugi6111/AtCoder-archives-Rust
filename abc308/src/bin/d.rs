...
fn main() {
    ...
    let snukesn = "snuke".to_string().as_bytes().to_owned();
    ...
    while let Some(x) = q.pop_front() {
        ...
        let ss = snukesn[(x.1 + 1) % 5] as char;
        if (x.0).0 > 0 && s[(x.0).0 - 1][(x.0).1] == ss {
            if seen.insert(((x.0).0 - 1, (x.0).1)) {
                q.push_back((((x.0).0 - 1, (x.0).1), (x.1 + 1) % 5));
            }
        }
        if (x.0).1 > 0 && s[(x.0).0][(x.0).1 - 1] == ss {
            if seen.insert(((x.0).0, (x.0).1 - 1)) {
                q.push_back((((x.0).0, (x.0).1 - 1), (x.1 + 1) % 5));
            }
        }
        if (x.0).0 < h - 1 && s[(x.0).0 + 1][(x.0).1] == ss {
            if seen.insert(((x.0).0 + 1, (x.0).1)) {
                q.push_back((((x.0).0 + 1, (x.0).1), (x.1 + 1) % 5));
            }
        }
        if (x.0).1 < w - 1 && s[(x.0).0][(x.0).1 + 1] == ss {
            if seen.insert(((x.0).0, (x.0).1 + 1)) {
                q.push_back((((x.0).0, (x.0).1 + 1), (x.1 + 1) % 5));
            }
        }
