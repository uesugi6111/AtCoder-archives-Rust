...
fn main() {
    ...
    let era = Eratosthenes::new(1000000).primes();
    let mut count = 0;
    for i in 0..era.len() - 2 {
        if era[i] * era[i] * era[i + 1] * era[i + 2] * era[i + 2] > n {
            break;
        }
        for j in i + 1..era.len() - 1 {
            if era[i] * era[i] * era[j] * era[j + 1] * era[j + 1] > n {
                break;
            }
            for k in j + 1..era.len() {
                if era[i] * era[i] * era[j] * era[k] * era[k] > n {
                    break;
                }
                count += 1;
            }
