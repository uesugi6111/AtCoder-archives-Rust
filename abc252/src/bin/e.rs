...
fn main() {
    ...
    let mut e = vec![vec![]; n];
    for (i, x) in abc.iter().enumerate() {
        e[x.0 - 1].push((x.1 - 1, x.2, i));
        e[x.1 - 1].push((x.0 - 1, x.2, i));
    }

    let mut set = std::collections::HashSet::new();

    dijk::dijkstra(&e, 0, &mut set);

    //let (_, a) = dijk::dijkstra(&e, 0, i, n);
    for i in set {
        print!("{} ", i + 1);
    }
}
mod dijk {
    ...
    impl Node {
        #[inline]
        pub fn new(position: usize, cost: i64, from: Option<(usize, usize)>) -> Self {
            Node {
                position,
                cost,
                from,
    ...
    pub fn dijkstra(
        ...
        let mut previous = vec![None; edge.len()];
        ...
        }) = nodes.pop()
        {
            ...
            edge[position]
                ...
                .for_each(|&(to, c, i)| {
                    nodes.push(Node::new(to, cost + c, Some((position, i))));
                });
        }
        None
    }
