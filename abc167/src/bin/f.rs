...
fn main() {
    ...
    input!(sc = sc, t: usize);
    for test in 0..t {
        input!(sc = sc, x: i64, y: i64, s: Chars);
        let mut dp = vec![vec![0i64; 2]; s.len() + 1];

        for i in 1..s.len() + 1 {
            if s[i - 1] == '?' {
                dp[i][0] = std::cmp::min(dp[i - 1][1] + y, dp[i - 1][0]); //c
                dp[i][1] = std::cmp::min(dp[i - 1][0] + x, dp[i - 1][1]); //j
            } else if s[i - 1] == 'C' {
                dp[i][0] = std::cmp::min(dp[i - 1][1] + y, dp[i - 1][0]); //c
                dp[i][1] = std::i32::MAX.into(); //j
            } else {
                dp[i][0] = std::i32::MAX.into(); //c
                dp[i][1] = std::cmp::min(dp[i - 1][0] + x, dp[i - 1][1]); //j
            }
        }
        println!(
            "Case #{}: {}",
            test + 1,
            std::cmp::min(dp[s.len()][0], dp[s.len()][1])
        );
    }
}
