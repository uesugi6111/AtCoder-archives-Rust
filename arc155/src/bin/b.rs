...
fn main() {
    ...
    input!(sc = sc, n: usize, a: i64, b: i64);

    let mut set = std::collections::BTreeSet::new();
    set.insert(a + b);
    set.insert(a - b);
    for _ in 0..n {
        input!(sc = sc, t: usize, a: i64, b: i64);
        ...
        if t == 1 {
            ...
            if let Some(x) = set.range(..a).last() {
                v.push(a - x);
            }
            if let Some(_) = set.range(a..=b).next() {
                v.push(0);
            }
            if let Some(x) = set.range(b..).next() {
                v.push(x - b);
            }
            v.sort();
            println!("{}", v[0]);
