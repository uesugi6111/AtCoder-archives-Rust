...
#[proconio::fastout]
fn main() {
    input!(n: usize, a: [i64; 3], d: [[i64; n]; n]);

    let mut v = vec![vec![]; n * 2];

    for i in 0..n {
        for j in 0..n {
            v[i].push((j, d[i][j] * a[0]));
            v[i].push((n + j, d[i][j] * a[1] + a[2]));
            v[n + i].push((n + j, d[i][j] * a[1] + a[2]));
        }
    }

    let a = dijk::dijkstra(&v, 0, n - 1).unwrap();
    let b = dijk::dijkstra(&v, 0, n + n - 1).unwrap();
