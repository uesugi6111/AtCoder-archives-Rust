...
fn main() {
    ...
    let edge = {
        let mut v = vec![vec![]; n];
        for (a, b, t, k) in abtk.into_iter() {
            v[a - 1 as usize].push((b - 1, t, k));
            v[b - 1 as usize].push((a - 1, t, k));
        }
        v
    };
    let ans = di::dijkstra(&edge, x - 1, y - 1).unwrap_or(-1);
    println!("{}", ans);
}
...
mod di {
    ...
    pub fn dijkstra(edge: &[Vec<(usize, i64, i64)>], start: usize, end: usize) -> Option<i64> {
