...
fn execute(n: i64) -> i64 {
    let mut v = vec![];

    for i in 0..n.to_string().chars().count() {
        v.push((n % 10i64.pow((i + 1) as u32)) / 10i64.pow(i as u32));
    }
    v.sort();

    let mut max = 0;
    for (i, c) in v.iter().enumerate() {
        max += *c * 10i64.pow(i as u32);
    }

    let mut min = 0;
    for (i, c) in v.iter().rev().enumerate() {
        min += *c * 10i64.pow(i as u32);
    }
