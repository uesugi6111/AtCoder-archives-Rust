...
fn main() {
    input!(n: Chars);
    let per = make_permutation(n.len());
    let mut max = 0;

    for i in per.iter() {
        let v = i.iter().map(|&x| n[x]).collect::<Vec<_>>();
        if v[0] == '0' {
            continue;
        }
        for j in 1..i.len() {
            ...
            let f = v.iter().take(j).collect::<String>().parse::<i64>().unwrap();
            let b = v.iter().skip(j).collect::<String>().parse::<i64>().unwrap();
