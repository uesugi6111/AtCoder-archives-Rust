...
#[proconio::fastout]
fn main() {
    input!(n: usize, m: usize, k: i64, uvw: [(usize, usize, i64); m]);
...
fn f(
    ...
    for &(to, w) in &e[now] {
        ...
        let buff = f(to, e, set, (cost + w) % k, k, n);
        min = min.min(buff);
    }
    min
}
...
pub mod mst {
    ...
    pub struct Edge(pub i64, pub i64, pub i128);

    pub fn kruskal(n: usize, edges: &[Edge], k: i128) -> i128 {
